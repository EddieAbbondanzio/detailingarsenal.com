...
namespace DetailingArsenal.Api.Users.Security {
    ...
    public class PermissionController : ControllerBase {
        ...
        public async Task<IActionResult> CreatePermission(PermissionCreateCommand command) {
            PermissionReadModel perm = await mediator.Dispatch<PermissionCreateCommand, PermissionReadModel>(command, User);
            return Ok(perm);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> UpdatePermission(Guid id, [FromBody] PermissionUpdateCommand command) {
            PermissionReadModel perm = await mediator.Dispatch<PermissionUpdateCommand, PermissionReadModel>(command, User);
            return Ok(perm);
        }
